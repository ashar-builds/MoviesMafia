...
| Browser page | [Pages/BrowserPage.xaml.cs](Pages/BrowserPage.xaml.cs) | Hosts the `WebView`, calls `AdBlockRuntime.InitializeAsync()`, then awaits a per-platform `ConfigureNativeWebViewAsync()` partial method before navigating. Shows a branded loading overlay at launch, hidden by `OnPageFinishedLoading` (raised from each platform's native navigation-completed hook) once the site is up. |
